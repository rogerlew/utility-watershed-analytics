...
| [20260718-db30a-production-legacy-base-adoption](20260718-db30a-production-legacy-base-adoption/package.md) | DB30A | `EXECUTED-HOLD-PUBLIC-ARTIFACT-SERVING` | Fresh encrypted backup, exact restore/export/rebuild, reviewed identity assignment, production adoption, rollback, fallback, and cleanup passed. The exact rehearsed rollback restored coherent `EMPTY` after the declared public artifact URI returned frontend HTML rather than the retained operator-owned immutable object; DB30B is the narrow unblock. |
