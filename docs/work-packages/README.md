...
| [20260718-db21a-legacy-base-tooling](20260718-db21a-legacy-base-tooling/package.md) | DB21A | `EXECUTED-COMPLETE` | Exact reviewed identity assignment, DB20-compatible content-addressed export including capability assets, offline fingerprint-equal rebuild, and atomic adoption/rollback/no-mutation negative proof passed disposable PostGIS without production access. |
| [20260718-db22-base-aware-planner](20260718-db22-base-aware-planner/package.md) | DB22 | `EXECUTED-COMPLETE` | Exact active-base assertion, stable-key change classification, removal refusal, DB09-valid forward/bound inverse/independent EMPTY plans, and deterministic synthetic proof passed; no production or publication action. |
| [20260718-db23-atomic-reconciler](20260718-db23-atomic-reconciler/package.md) | DB23 | `EXECUTED-COMPLETE` | Exact reviewed-base/plan reassertion, locked atomic keyed reconciliation, stable child IDs, run replacement, exact removals, capability/pointer switch, rollback, and old/new reader visibility passed disposable PostGIS proof. |
| [20260718-db24-reconciler-resilience](20260718-db24-reconciler-resilience/package.md) | DB24 | `EXECUTED-COMPLETE` | Exact read-only no-op, forward-bound inverse rollback, expired-attempt recovery command, post-pointer rollback, and DB20 clean-build/DB23 reconciliation equivalence passed synthetic disposable PostGIS proof. |
| [20260718-db25-deployment-serialization](20260718-db25-deployment-serialization/package.md) | DB25 | `EXECUTED-COMPLETE` | Shared production workflow/host serialization, explicit separately credentialed migration, executable code/schema/release compatibility, and seven-role negative/rotation proof passed disposable forest1 acceptance. |
| [20260718-db26-deploy-database-orchestrator](20260718-db26-deploy-database-orchestrator/package.md) | DB26 | `EXECUTED-COMPLETE` | Durable locked oneshot orchestration, immutable resume coordinates, exact no-op/backup/apply/smoke/rollback/report ordering, private state, forest1 report archive contract, retention, alerting, and interruption/crash/reboot/failure rehearsals passed without production access. |
| [20260718-db27-protected-release-workflow](20260718-db27-protected-release-workflow/package.md) | DB27 | `EXECUTED-COMPLETE` | Deterministic release authorization, manual-only distinct deploy/rollback environments, durable report retention, aggregate no-store active status, private capacity/age/attempt monitoring, and synthetic bypass/failure rehearsal passed without workflow dispatch or production access. |
| [20260718-db27a-production-compatibility-rollout](20260718-db27a-production-compatibility-rollout/package.md) | DB27A | `EXECUTED-COMPLETE` | Fresh encrypted `wepp3` backup to `forest1:/wc1`, exact isolated forward/rollback rehearsal, additive one-shot migrations, least-privilege roles, protected workflow deployment, unchanged database identity/data/API, and complete cleanup passed. |
| [20260718-db28-gate-victoria-release-inputs](20260718-db28-gate-victoria-release-inputs/package.md) | DB28 | `EXECUTED-COMPLETE` | Exact Gate Creek and 31-member Victoria ordinary inputs plus Gate dynamic and Sooke09/Sooke15 precomputed RHESSys assets were published immutably to the operator-owned `forest1:/wc1` store; schemas, counts, joins, representative reads, private modes, full object hashing, and zero-upstream exact replay passed without production access or activation. |
| [20260718-db29-mill-creek-rhessys-revendor](20260718-db29-mill-creek-rhessys-revendor/package.md) | DB29 | `EXECUTED-HOLD-RHESSYS-SOURCE` | Exact `some-oligopoly/disturbed9002_wbt` ordinary inputs were published immutably and passed counts, joins, schemas, private-store proof, and zero-upstream replay. Completion is held because the successor has no RHESSys tree, all 56 registered map coordinates return 404, both public catalog and `/wc1` contain no accepted source, and no empty/inferred/former-run capability was substituted. |
| [20260718-db30-nasa-bremerton-inputs](20260718-db30-nasa-bremerton-inputs/package.md) | DB30 | `EXECUTED-COMPLETE` | Exact 93-member NASA successor and approved Bremerton01–03 inputs were published immutably to operator-owned `forest1:/wc1`; enrichment lineage, materialized multipart counts, 288 Parquet joins, schemas, full storage hashing, and zero-upstream replay passed. Bremerton04 is an explicit operator-reviewed exclusion because all five required child products are absent. |
| [20260718-db30a-production-legacy-base-adoption](20260718-db30a-production-legacy-base-adoption/package.md) | DB30A | `EXECUTED-COMPLETE` | Fresh encrypted backup, exact restore/export/rebuild, reviewed identity assignment, production adoption, rollback, fallback, and cleanup passed. DB30B closed the historical serving hold by exposing the retained namespace and re-adopting the unchanged baseline. |
| [20260719-db30b-self-hosted-public-artifact-serving](20260719-db30b-self-hosted-public-artifact-serving/package.md) | DB30B | `EXECUTED-COMPLETE` | Tailscale-only forest1 origin and exact production proxy now serve the existing operator-owned `/wc1` namespace without a provider. Fresh backup, failure-triggered rollback, corrected re-adoption, exact public reads/ranges, materialized capabilities, persistence, restart/reload, and cleanup passed with unchanged serving rows. |
| [20260719-db31-first-release-candidate](20260719-db31-first-release-candidate/package.md) | DB31 | `EXECUTED-COMPLETE` | Exact 129-member target inputs were canonicalized immutably, two independent empty builds produced identical fingerprints/counts and passed application/capability checks, and DB09-valid forward/inverse/EMPTY plans were retained by hash against the re-observed DB30A base. |
