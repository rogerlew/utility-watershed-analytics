...
| [20260716-db03-production-runtime-convergence](20260716-db03-production-runtime-convergence/package.md) | DB03 | `EXECUTED-HOLD-PUBLISH` | Host convergence, exact identity, app-only rollback, safe unit cycle, socket hardening, cross-principal lock, and canonical locked backup passed. The agent branch is published, but the idle runner remains disabled until the safe workflow is merged to `main` and verified. |
| [20260717-db03a-production-runner-ownership-closure](20260717-db03a-production-runner-ownership-closure/package.md) | DB03A | `EXECUTED-HOLD-PRIVILEGE` | Safe fork `main` publication passed with no queued run and production unchanged; secret delivery and new runner installation stopped before mutation because noninteractive sudo is unavailable. |
| [20260716-db04-legacy-loader-guardrails](20260716-db04-legacy-loader-guardrails/package.md) | DB04 | `SCAFFOLDED` | DB02 dependency is complete; repository implementation is unblocked, while production deployment follows DB03. |
