...
run_executor python manage.py rollback_legacy_adoption \
    ...
    | tee "$evidence/manual-rollback-result.json"
run_executor python /ops.py state \
    --input-root /reviewed/db28 \
    --artifact-root /artifacts \
    | tee "$evidence/manual-rollback-state.json"

python3 - "$evidence/manual-rollback-state.json" <<'PY'
import json
import sys
...
printf 'DB30B production database rollback complete.\n'
